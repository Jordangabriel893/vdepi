...
export class FaturaComponent implements OnInit, OnDestroy {
  ...
  onSearch(){
     let value = this.queryField.value.replace('.', '').replace('-', '').replace('/', '').toLowerCase();
      ...
      this.faturasFiltradas =
        this.faturas.filter(x => x.nomeArrematante.toLowerCase().includes(value) ||
                                  x.cpfArrematante.replace('.', '').replace('-', '').replace('/', '').includes(value) ||
                                  x.itens[0].descricao.replace('.', '').replace('-', '').replace('/', '').includes(value)
                                  );
  }
  filtrarFaturas(){
  ...
  if(statusControl == '0'){
   ...
   const faturasEncontradas =  this.faturasFiltradas.filter(fatura => fatura.status == statusControl)
   if(faturasEncontradas.length == 0 ){
    this.notifierService.notify('error', 'Nenhuma fatura encontrada');

   }else{
    this.faturasFiltradas = faturasEncontradas
   }
