...
@Component({
  selector: 'app-create-reu',
  templateUrl: './create-reu.component.html',
  styleUrls: ['./create-reu.component.scss']
})
export class CreateReuComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.restangular.all('judicial/reu').post(body).subscribe(a => {
      this.notifierService.notify('success', 'Reu criado com sucesso');
      ...
      error => {
        ...
        for (const k in errors) {
          if (k.toLowerCase() === 'exception') {
            this.notifierService.notify('error', 'Erro ao criar reu');
          } else {
