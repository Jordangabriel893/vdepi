...
export class ReuComponent implements OnInit {
  ...
  ngOnInit() {
    this.restangular.one('judicial/reu').get().subscribe((response) => {
  ...
  edit(id) {
    this.router.navigate(['/update-reu', id], { relativeTo: this.route });
  }
  ...
  remove(id){
    ...
    .subscribe((ans: ResolveEmit) => {
      if (ans.resolved) {
        this.restangular.one('judicial/reu', id).remove().subscribe((response) => {
