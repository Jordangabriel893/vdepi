...
export class CreateAutorComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.restangular.all('judicial/autor').post(body).subscribe(a => {
      this.notifierService.notify('success', 'autor criado com sucesso');
      ...
      error => {
        ...
        for (const k in errors) {
          if (k.toLowerCase() === 'exception') {
            this.notifierService.notify('error', 'Erro ao criar autor');
          } else {
