...
@Component({
  selector: 'app-create-credor',
  templateUrl: './create-credor.component.html',
  styleUrls: ['./create-credor.component.scss']
})
export class CreateCredorComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.restangular.all('judicial/credor').post(body).subscribe(a => {
      this.notifierService.notify('success', 'credor criado com sucesso');
      ...
      error => {
        ...
        for (const k in errors) {
          if (k.toLowerCase() === 'exception') {
            this.notifierService.notify('error', 'Erro ao criar credor');
          } else {
