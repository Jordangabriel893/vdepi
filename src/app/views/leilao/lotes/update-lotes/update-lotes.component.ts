...
export class UpdateLotesComponent implements OnInit {
  ...
  preencherCampoJudicial(dados){
    ...
    return {
      ...
      juizes: [juizes ? dados.loteJudicial.juizes.map((x) => x.juizId) : []],
      escrivaes: [escrivaes ? dados.loteJudicial.escrivaes.map((x) => x.escrivaoId) : []],
      autores: [autores ? dados.loteJudicial.autores.map((x) => x.autorId) : []],
      reus: [reus ? dados.loteJudicial.reus.map((x) => x.reuId) : []],
      fieisDepositarios: [fieisDepositarios ? dados.loteJudicial.fieisDepositarios.map((x) => x.fielDepositarioId) : []],
      partes: [partes ? dados.loteJudicial.partes.map((x) => x.parteId) : []],
      credores: [credores ? dados.loteJudicial.credores.map((x) => x.credorId) : []],
    }
  }
  ...
  updateForm(dados, fotos) {
    ...
    this.formulario.get('loteJudicial').get('juizoId').valueChanges.subscribe((value) => {
      ...
      this.restangular.one('judicial/vara').get({juizoId: value}).subscribe((allResp: any) => {
        this.varas = allResp.data;
      });
    });

    this.formulario.get('loteJudicial').get('varaId').valueChanges.subscribe((value) => {
      if(isNullOrUndefined(value)) {
