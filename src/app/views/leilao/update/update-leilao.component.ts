...
export class UpdateLeilaoComponent implements OnInit {
  ...
  updateForm(dados) {
    ...
    const arrayRegraHabiliacaoId = dados.regrasHabilitacao.map(
      (regra) => regra.regraHabilitacao.regraHabilitacaoId
    );
    ...
    this.formulario = this.formBuilder.group({
      ...
      dataLeilao: [
        dados.dataLeilao ? moment.utc(dados.dataLeilao).local().toDate() : null,
      ],
      dataAberturaLance: [
        moment.utc(dados.dataAberturaLance).local().toDate(),
        Validators.required,
      ],
      dataInicioAgendamento: [
        dados.dataInicioAgendamento
          ? moment.utc(dados.dataInicioAgendamento).local().toDate()
          : null,
      ],
      dataFimAgendamento: [
        dados.dataFimAgendamento
          ? moment.utc(dados.dataFimAgendamento).local().toDate()
          : null,
      ],
      dataEdital: [
        dados.dataEdital ? moment.utc(dados.dataEdital).local().toDate() : null,
      ],
      dataNotificacao: [
        dados.dataNotificacao
          ? moment.utc(dados.dataNotificacao).local().toDate()
          : null,
      ],
      dataInicioLiberacao: [
        dados.dataInicioLiberacao
          ? moment.utc(dados.dataInicioLiberacao).local().toDate()
          : null,
      ],
      dataFimLiberacao: [
        dados.dataFimLiberacao
          ? moment.utc(dados.dataFimLiberacao).local().toDate()
          : null,
      ],
      dataDiarioOficial: [
        dados.dataDiarioOficial
          ? moment.utc(dados.dataDiarioOficial).local().toDate()
          : null,
      ],
        ...
        dados.pracas
          ? dados.pracas.map((x) =>
              this.formBuilder.group({
                dataExecucao: [
                  moment.utc(x.dataExecucao).local().toDate(),
                  Validators.required,
                ],
                pracaLeilaoId: [x.pracaLeilaoId, Validators.required],
                numeroPraca: [x.numeroPraca, Validators.required],
    ...
    if (dados.configuracaoParcela && dados.lanceParcelado) {
      this.formulario.addControl(
        ...
        this.formBuilder.group({
          ...
          maximoParcelas: [
            dados.configuracaoParcela.maximoParcelas,
            Validators.required,
          ],
          indiceCorrecaoId: [
            dados.configuracaoParcela.indiceCorrecao.map(
              (x) => x.indiceCorrecaoId
            ),
            Validators.required,
          ],
        })
