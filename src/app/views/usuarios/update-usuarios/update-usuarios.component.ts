...
export class UpdateUsuariosComponent implements OnInit {
  ...
  loadingLotesArrematados = true;
  loadingFaturas = true;
  loadingBloqueios = true;
  loadingLances = true;
  constructor(
  ...
  mostrarQuandoPerfil(...args: string[]) {
    const perfis = this.perfis
      .filter((x) => args.includes(x.descricao))
      .map((x) => x.perfilUsuarioId);
    return this.formulario.controls.perfis.value.some((x) =>
      perfis.includes(x)
    );
  }
