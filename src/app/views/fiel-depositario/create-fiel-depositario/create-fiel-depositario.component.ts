...
@Component({
  selector: 'app-create-fiel-depositario',
  templateUrl: './create-fiel-depositario.component.html',
  styleUrls: ['./create-fiel-depositario.component.scss']
})
export class CreateFielDepositarioComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.restangular.all('judicial/fielDepositario').post(body).subscribe(a => {
      this.notifierService.notify('success', 'fielDepositario criado com sucesso');
      ...
      error => {
        ...
        for (const k in errors) {
          if (k.toLowerCase() === 'exception') {
            this.notifierService.notify('error', 'Erro ao criar fielDepositario');
          } else {
